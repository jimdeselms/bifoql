...
docker push 779051441487.dkr.ecr.eu-west-1.amazonaws.com/bifoql:$tag
...
#docker run --rm --entrypoint kubectl --mount type=bind,source=$currPath,destination=/workspace -w /workspace 779051441487.dkr.ecr.eu-west-1.amazonaws.com/$($kubectlName):latest create -f deployments/deployment-bifoql.yaml

# In case we made updates to the deployment file, run apply

docker run --rm --entrypoint kubectl --mount type=bind,source=$currPath,destination=/workspace -w /workspace 779051441487.dkr.ecr.eu-west-1.amazonaws.com/$($kubectlName):latest apply -f deployments/deployment-bifoql.yaml

# Now perform rolling-update to latest version (modify the version tag on the docker image spec)

docker run --rm --entrypoint kubectl --mount type=bind,source=$currPath,destination=/workspace -w /workspace 779051441487.dkr.ecr.eu-west-1.amazonaws.com/$($kubectlName):latest set image deployment bifoql bifoql=779051441487.dkr.ecr.eu-west-1.amazonaws.com/bifoql:$tag --record
